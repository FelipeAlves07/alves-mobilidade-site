...
export default function QuemSomosPage() {
  return (
    <main className="bg-[#151515] text-white">
      ...
      <section className="bg-[#202020] px-5 py-20 md:py-28 lg:px-8"><div className="mx-auto grid max-w-7xl gap-6 md:grid-cols-2 xl:grid-cols-4">{values.map((item)=>{const Icon=item.icon; return <div key={item.title} className="premium-card rounded-[2rem] p-8"><div className="mb-6 inline-flex rounded-2xl bg-[#d6a85f]/10 p-4 text-[#f1d28b]"><Icon size={26}/></div><h3 className="text-2xl font-black">{item.title}</h3><p className="mt-4 leading-8 text-zinc-400">{item.description}</p></div>})}</div></section>
    </main>
  );
}
