...
import { createBatchExecutorDepsForClient, resumeBatchRun } from "@/lib/batch-executor";
...
/**
 * POST — retoma o lote pausado: reabilita os claims (apenas empresas
 * `pendente` são reclamadas; nunca `concluida`/`sem_dados`/`falha`).
 */
export async function POST(
  ...
  try {
    ...
    const run = await resumeBatchRun(id, createBatchExecutorDepsForClient(client));
