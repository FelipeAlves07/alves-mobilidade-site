...
export async function geocodeServer(query: string): Promise<{ lat: number; lon: number; label: string } | null> {
...
export async function fetchRouteServer(origin: string, destination: string): Promise<RouteInfo | null> {
  if (!ORS_KEY) return null;

  const [orig, dest] = await Promise.all([geocodeServer(origin), geocodeServer(destination)]);
